...
exports.createRazorpayOrder = async (req, res) => {
  try {
    ...
    res.json({
      ...
      keyId: process.env.RAZORPAY_KEY_ID // Send key_id to frontend (safe to expose)
    });
  } catch (error) {
    ...
    if (razorpay_signature === expectedSign) {
      ...
      try {
        ...
        const [orderResult] = await connection.query(
          `INSERT INTO orders (user_id, address_id, total_amount, payment_method, payment_status, status, razorpay_order_id, razorpay_payment_id) 
           VALUES (?, ?, ?, 'razorpay', 'completed', 'processing', ?, ?)`,
          [userId, addressId, totalAmount, razorpay_order_id, razorpay_payment_id]
        );
